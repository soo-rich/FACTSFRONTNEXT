...
const UserDetails = ({ user: userData }: { user: UtilisateurDto }) => {
  ...
  useEffect(() => {
    ...
    if (image) {
      Utilsmethod.getFileFormApi(image.storageKey, image.provider).then(value => {
        if (value) {
          ...
          <div>
            ...
            <div className='flex flex-col gap-2'>
              ...
              <div className='flex items-center flex-wrap gap-x-1.5'>
                ...
                <Chip
                  label={userData?.isActive ? 'Activer' : 'Desactiver'}
              ...
              <div className='flex items-center flex-wrap gap-x-1.5'>
                ...
                <Typography color='text.primary'>{userData?.numero}</Typography>
              </div>
              <div className='flex items-center flex-wrap gap-x-1.5'>
                <Typography className='font-medium' color='text.primary'>
                  Inscription:
                </Typography>
                <Typography color='text.primary'>{Utilsmethod.formatDate(userData?.createdat)}</Typography>
              </div>
            </div>
          </div>
          <Divider className='mlb-4' />
