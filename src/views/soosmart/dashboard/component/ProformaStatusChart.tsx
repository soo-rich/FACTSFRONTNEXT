...
const COLORS = [
  ...
  { key: 'accepted', label: 'Acceptées', color: '#00d4bd' },
  { key: 'rejected', label: 'Rejetées', color: '#ff4c51' }
]
...
const ProformaStatusChart = () => {
  const { data, isLoading, isError } = useQuery({
    queryKey: [StatAPIService.STAT_KEY, 'proforma', 'status'],
    queryFn: StatAPIService.getProformaStatus,
  ...
  return (
    <Card className='h-full'>
      <CardHeader title='Statut des Proformas' subheader={`Taux d'adoption: ${data.adoptionRate.toFixed(1)}%`} />
      <CardContent>
        <AppRecharts>
...
export default ProformaStatusChart
